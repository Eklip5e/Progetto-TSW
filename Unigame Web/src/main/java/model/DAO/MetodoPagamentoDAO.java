...
public class MetodoPagamentoDAO implements MetodiDAO<Fatturazione> {

    @Override
    public void doSave(Fatturazione fatturazione) {
        try (Connection con = ConPool.getConnection()) {
            PreparedStatement ps = con.prepareStatement(
                    "INSERT INTO MetodoPagamento (numero, titolare, scadenza, cvc, idUtente) VALUES (?, ?, ?, ?, ?)",
    ...
    public void doDelete(int idFatturazione) {
        try (Connection con = ConPool.getConnection()) {
            PreparedStatement ps = con.prepareStatement(
                    "DELETE FROM MetodoPagamento WHERE idFatturazione = ?");
    ...
    public void doUpdate(Fatturazione fatturazione, int id) {
        try (Connection con = ConPool.getConnection()) {
            PreparedStatement ps = con.prepareStatement(
                    "UPDATE MetodoPagamento SET numero = ?, titolare = ?, scadenza = ?, cvc = ?, idUtente = ? WHERE idFatturazione = ?");
            ps.setString(1, fatturazione.getNumero());
            ps.setString(2, fatturazione.getTitolare());
            ps.setDate(3, new Date(fatturazione.getScadenza().getTime()));
            ps.setString(4, fatturazione.getCvc());
    ...
    public Fatturazione doRetrieveById(int id) {
        try (Connection con = ConPool.getConnection()) {
            PreparedStatement ps = con.prepareStatement(
                    "SELECT * FROM MetodoPagamento WHERE idFatturazione = ?");
            ps.setInt(1, id);
    ...
    public Fatturazione doRetrieveByNumeroAndUtente(String numeroCarta, int idUtente) {
        try (Connection con = ConPool.getConnection()) {
            PreparedStatement ps = con.prepareStatement(
                    "SELECT * FROM MetodoPagamento WHERE numero = ? AND idUtente = ?");
